...
That seems like small-sample noise and a stable team profile — slight movement year to year that doesn’t clear the bar. Roster continuity, opponent mix and a few close, low-event games can make it harder to see a real change.***
...
```{r}
net <- lm(differential ~ homeaway, data = terps25)
summary(net)
```
ANSWER*****

Maryland men’s soccer has been a different team depending on where they’ve played. On the road, the Terps have struggled, often leaving empty-handed after one- or two-goal losses. But when they’ve stepped onto Ludwig Field, the results have flipped — comfortable wins and more consistent attacking play. A story could dig into how home crowds, travel demands, and player confidence have shaped the Terps’ season, illustrated through key examples like their narrow defeats away from College Park versus the sharper finishing and defensive stability they’ve shown at home.
...
```{r}
colnames(matches_2025)
```

```{r}
model <- lm(differential ~ sh_att + so_g + corners + defensive_so_g + defensive_fouls, data = matches_2025)

summary(model)
```

***
I chose variables that directly measure offensive and defensive performance — shots on goal, shots on goal allowed, fouls, shot attempts, and corners — because they best capture a team’s ability to create and prevent quality scoring chances. The model showed that shots on goal and shots on goal allowed were by far the strongest predictors of goal differential (Adjusted R² = 0.574), with residuals centered near zero and ranging roughly from -7 to +7, indicating the model is both unbiased and reasonably accurate given the variability of soccer matches. *** 
...
matches_2025_clean <- matches_2025 |>
  filter(!is.na(sh_att), 
         !is.na(so_g), 
         !is.na(corners), 
         !is.na(defensive_so_g), 
         !is.na(defensive_fouls))
```

```{r}
matches_2025_clean <- matches_2025_clean |>
...
matches_2025_clean |>
  ...
  select(date, team, opponent, differential, predicted, residuals) |>
  head()
```
...
*** The outlier games show that some teams either lost by much more than the model predicted, like Vermont drawing instead of winning, Iona losing when favored, or won by far larger margins than expected, like Alcorn and Xavier blowouts. These results highlight matches where finishing, goalkeeping, or other unmeasured factors swung outcomes beyond what offensive and defensive shot-based stats could explain. ***

```{r}
maryland_residuals <- matches_2025_clean |>
  filter(team == "Maryland Terrapins, Terps") |>
  arrange(residuals) |>
  select(date, team, opponent, differential, predicted, residuals)
```
...
***Maryland’s biggest over-performance came in a 1–0 win against Purdue, where the model predicted a loss, while its worst under-performances were lopsided defeats to Penn State and USC that were more severe than expected. This suggests the Terps can exceed expectations in tight Big Ten games but still struggle to keep pace with nationally powerhouse opponents.***


***What surprised me most was how strongly shots on goal for and against predicted outcomes. If I were writing a article, I’d want to explore why Maryland tends to over-perform in close conference games but falters against top national programs, and whether that reflects talent gaps, depth, or game management.***